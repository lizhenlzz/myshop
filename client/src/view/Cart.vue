<template>
  <div>
      ...
      <div class="cart-foot-wrap">
        <div class="cart-foot-inner">
          ...
          <div class="cart-foot-r">
            ...
            <div class="btn-wrap">
              <a class="btn btn--red" @click="checkOut">去结帐</a>
            </div>
          </div>
        </div>
      </div>
    ...
    <modal :mdShow="modalconfirm">
      ...
      <div slot="btnGroup">
        <a href="javascipt:;" class="btn btn--m" @click="delCart">确定</a>
        <a href="javascipt:;" class="btn btn--m" @click="modalconfirm=false">取消</a>
      </div>
    </modal>
  </div>
</template>
<script>
  ...
  export default{
    ...
    methods:{
        getCartList(){
            this.$http.post('/users/cart').then((result)=>{
        ...
        //全选
        toggleCheckAll(){
            let flag=!this.checkAllFlag;//默认非全选状态
            this.cartList.forEach((item)=>{
                item.checked=flag ? 1 : 0   //判断商品是否全选  如果不是全选让它为true  反之亦然
            })
            //将是否全选传递给后台
        ...
        //结账
        checkOut(){
            if(this.checkedCount>0){//如果选中商品数量大于0，改变路由
                this.$router.push({
                  path:'/address'
                })
            }else{
